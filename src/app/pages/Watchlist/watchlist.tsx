...
import watchstyle from "./watchlist.module.scss";
import '../MediaCenter/mediacenter.scss';
...
import { selecteduserWatchlistDelete, selecteduserMovieBasket, totalitemFunction, favoritecountFunction, watchcountFunction } from "../../features/CardSlice";

import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faCircleXmark } from '@fortawesome/free-solid-svg-icons';
import { faListAlt } from '@fortawesome/free-solid-svg-icons';
import MoviebuttonProp from "../MediaCenter/mediabuttonprops";
...
import Watchprop from "./watchlistprops";

const WatchlistPage:React.FC<any> = ({translate}) => {
   ...
   const deletewatchlist = (movieId:number) => {
    dispatch(selecteduserWatchlistDelete([updateselectuser, movieId]));
    localStorage.setItem('storageselectusermovie', JSON.stringify(updateselectuser));
    setWatchcount(updateselectuser.watchlist.length);
    updateapi();
    getUserListforBasket();
   }
   ...
   const updateapi = () => {
    ...
    const addOrRemoveCard = (movieId:number, movieCard:string) => {
        ...
        const filterprd = movieproducts.find(row => row.id == movieId)
        
        dispatch(selecteduserMovieBasket([updateselectuser, movieId, filterprd, movieCard]))
        localStorage.setItem('storageselectusermovie', JSON.stringify(updateselectuser));
        setTotalitem(updateselectuser.card.map(x => x.quantity))

       updateapi();
        // const finduserinfo = allusers.find(row => row.id == updateselectuser.id)
    ...
    return (
       <>
        <div className={watchstyle.section1}>
            <div className={watchstyle.section1_header}>
                <h1>{contnt.watchh11}</h1>
                <div className={watchstyle.section1_header_corn}>
                    <img src={popcornimg} />
                </div>
            </div>
            <div className={watchstyle.container1}>
                ...
                <div className={watchstyle.container1_watchlist_movie}>
                {
                    updateselectuser.watchlist?.length === 0 ? <h1>{contnt.watchh12}</h1> :
                            showmovie == 'firstinfo' ? 
                           updateselectuser.watchlist?.map(row => {
                               return(
                                   <>
                                        <Watchprop deletewatch={() => deletewatchlist(row.id)} addRemoveCard={() => addOrRemoveCard(row.id, row.category)} image={row.image} name={row.name} />
                                   </>
                               )
                           })
                            ...
                            updateselectuser.watchlist?.filter(row => row.category == "Film")?.map(row => {
                                return(
                                    <>
                                        <Watchprop deletewatch={() => deletewatchlist(row.id)} addRemoveCard={() => addOrRemoveCard(row.id, row.category)} image={row.image} name={row.name} />
                                    </>
                            ...
                            showmovie == 'thirdinfo' ? 
                            updateselectuser.watchlist?.filter(row => row.category == "Series")?.map(row => {
                                return(
                                    <>                                  
                                        <Watchprop deletewatch={() => deletewatchlist(row.id)} addRemoveCard={() => addOrRemoveCard(row.id, row.category)} image={row.image} name={row.name} />
                                    </>
                            ...
                            showmovie == 'fourinfo' ? 
                            updateselectuser.watchlist?.filter(row => row.category == "Documentary")?.map(row => {
                                return(
                                    <>
                                        <Watchprop deletewatch={() => deletewatchlist(row.id)} addRemoveCard={() => addOrRemoveCard(row.id, row.category)} image={row.image} name={row.name} />
                                    </>
                            ...
                            updateselectuser.watchlist?.filter(row => row.category == "Kids")?.map(row => {
                                return(
                                    <>
                                        <Watchprop deletewatch={() => deletewatchlist(row.id)} addRemoveCard={() => addOrRemoveCard(row.id, row.category)} image={row.image} name={row.name} />
                                    </>
